...
public class Monster {
    ...
    public static Monster createCrawler()        { return new Monster(TYPE_CRAWLER,           "Crawler",            30, 40,  4); }
    public static Monster createSpitter()        { return new Monster(TYPE_SPITTER,            "Spitter",            44, 40,  5); }
    public static Monster createNormalZombie()   { return new Monster(TYPE_NORMAL_ZOMBIE,      "Zombie",             40, 50,  4); }
    public static Monster createScreamer()       { return new Monster(TYPE_SCREAMER,           "Screamer",           32, 30,  3); }
    public static Monster createLaBronxGangster(){ return new Monster(TYPE_LA_BRONX_GANGSTER,  "La Bronx Gangster",  50, 40,  6); }
    public static Monster createCIAOfficer()     { return new Monster(TYPE_CIA_OFFICER,        "CIA Officer",        70, 65,  9); }
    public static Monster createInfectedCIA()    { return new Monster(TYPE_INFECTED_CIA,       "Infected CIA Agent", 80, 70,  6); }
    public static Monster createBilly()          { return new Monster(TYPE_BILLY,              "Billy",              60, 60,  8); }
    ...
    public String takeDamage(int damage) {
        if (!alive) return name + " is already dead.";
        currentHP -= damage;
        if (currentHP <= 0) {
            currentHP = 0;
            alive     = false;
    ...
    public String[] performAttack(boolean billyPresent) {
        if (!alive) return new String[]{ name + " is dead and cannot attack.", "0", "NONE" };
        ...
        if (!hit) {
            return new String[]{ name + " attacks " + targetName + " but misses!", "0", target };
        }
        return new String[]{ name + " hits " + targetName + " for " + damage + " damage!", String.valueOf(damage), target };
    }
    ...
    public String[] companionAttack() {
        if (!isCompanion || !alive) return new String[]{ "", "0", "ENEMY" };
        return new String[]{
    ...
    public void incrementTurnsInRoom() {
        if (monsterType.equals(TYPE_SCREAMER) && alive) turnsInRoom++;
    }
    ...
    public void resetShriekCounter() { turnsInRoom = 0; }

    // -------------------------------------------------------
    // LOOT
    // -------------------------------------------------------

    public void addLoot(Item item) { lootTable.add(item); }
    ...
    public String interactWithBilly() {
        ...
        if ((double) currentHP / maxHP < 0.25) return "Billy sways slightly, barely holding himself together.";
    ...
    public void markKilledByPlayer() {
        if (monsterType.equals(TYPE_BILLY)) billyKilledByPlayer = true;
    }
